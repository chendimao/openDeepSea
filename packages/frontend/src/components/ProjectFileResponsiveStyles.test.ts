...
test('file management styles keep source controls usable on desktop and mobile', () => {
  ...
  assert.match(css, /@media \(max-width: 76[78]px\)\s*\{[\s\S]*?\.files-filters\s*\{[^}]*flex-wrap:\s*wrap/s);
  assert.match(css, /@media \(max-width: 76[78]px\)\s*\{[\s\S]*?\.files-filter-select\s*\{[^}]*flex:\s*1 1 128px/s);
  assert.match(css, /@media \(max-width: 76[78]px\)\s*\{[\s\S]*?\.file-preview-details\s*\{[^}]*grid-template-columns:\s*1fr/s);
  assert.match(css, /@media \(max-width: 76[78]px\)\s*\{[\s\S]*?\.project-file-action-button\s*\{[^}]*max-width:\s*100%/s);
