...
test('image job form exposes the backend-supported count maximum', () => {
  const html = renderWithQueryClient(
    <ImageJobFormView
      projectId="project-1"
      state={createEmptyImageJobFormState()}
      busy={false}
      error={null}
      onStateChange={() => undefined}
      onSubmit={() => undefined}
...
test('image job payload trims prompt and includes selected source file ids only for image-to-image', () => {
  const payload = buildImageJobPayload({
    ...
    prompt: '  生成赛博风格海报  ',
    sourceFiles: [
      createProjectFile({ id: 'file:file-1', original_name: 'source.png', mime_type: 'image/png' }),
