...
test('SessionShell renders workflow chat message with gate and agent summaries', () => {
  ...
  assert.match(workflowMessageArea, /data-workflow-assignment-summary="true"/);
  assert.doesNotMatch(workflowMessageArea, /data-workflow-view-toggle="true"/);
  assert.doesNotMatch(workflowMessageArea, /data-session-workflow-map="mission"/);
  assert.doesNotMatch(workflowMessageArea, /data-workflow-flow-root="true"/);
  assert.doesNotMatch(workflowMessageArea, /flow-path-parallel/);
  assert.match(workflowMessageArea, /等待 plan gate/);
  assert.match(workflowMessageArea, /1 个门禁/);
  assert.match(workflowMessageArea, /Codex/);
  assert.doesNotMatch(workflowMessageArea, /data-card-tone="agent"/);
  assert.doesNotMatch(workflowMessageArea, /data-card-tone="gate"/);
  assert.doesNotMatch(workflowMessageArea, /deepsea-workflow-state-step/);
  assert.doesNotMatch(workflowMessageArea, /data-workflow-state-stream="true"/);
  assert.doesNotMatch(workflowMessageArea, /Execution Log 合并事件/);
  assert.match(workflowMessageArea, /data-workflow-artifact-action="approve"/);
...
test('SessionShell renders merged workflow events as a readable state stream without controller data', () => {
  const payload = createPayload();
  const now = Date.now();
  payload.activeSession.runs = [];
  payload.activeSession.agentEvents = [];
  ...
  payload.activeSession.messages = [
    {
      id: 'msg-workflow-recovery',
      session_id: payload.activeSession.session.id,
      role: 'system',
      sender_id: 'workflow',
      sender_name: '工作流',
      content: '产品经理检测到子任务异常：Superpowers TDD evidence gate requires RED failed and GREEN passed records。',
      message_type: 'text',
      status: 'completed',
      metadata: JSON.stringify({ event_type: 'workflow_recovery_decided', workflow_run_id: 'workflow-run-current' }),
      created_at: now,
    },
    {
      ...
      sender_id: 'reviewer',
      sender_name: '审查员',
      content: '```json\n{ "verdict": "pass", "findings": [], "requiredFixes": [] }\n```',
      message_type: 'text',
      status: 'completed',
      metadata: JSON.stringify({ workflow_run_id: 'workflow-run-current', agent_run_id: 'review-run-1' }),
      created_at: now + 1_000,
    },
    {
      id: 'msg-workflow-verify',
      session_id: payload.activeSession.session.id,
      role: 'system',
      sender_id: 'workflow',
      sender_name: '工作流',
      content: '任务「删除项目前停止 active runs」进入 verify 阶段。',
      message_type: 'text',
      status: 'completed',
      metadata: JSON.stringify({ event_type: 'workflow_stage_changed', workflow_run_id: 'workflow-run-current' }),
      created_at: now + 2_000,
    },
  ];
...
test('SessionShell summarizes workflow planner JSON as reply text and assignment structure', () => {
  ...
  payload.activeSession.messages = [
    ...
    },
    {
      ...
      message_type: 'system',
      status: 'completed',
      metadata: JSON.stringify({ event_type: 'workflow_started', workflow_run_id: 'workflow-run-json-summary' }),
      created_at: now + 1_000,
    },
    {
      id: 'msg-planner-json-summary',
      session_id: payload.activeSession.session.id,
      role: 'assistant',
      sender_id: 'planner',
      sender_name: '规划师',
      content: [
        ...
        '  "steps": [',
        '    {',
        '      "title": "定位项目删除与 active runs 校验链路",',
        '      "intent": "阅读后端项目删除 API、服务/仓储逻辑、run/task 状态模型和当前报错来源。",',
        '      "assigneeRole": "analyst",',
        '      "preferredBackend": "codex",',
        '      "scopeRead": ["packages/backend/src"],',
        '      "scopeWrite": [],',
        '      "acceptance": ["明确 project has active runs 的具体抛出位置。"],',
        '      "dependsOn": []',
        '    },',
        '    {',
        '      "title": "设计并实现删除前停止项目任务",',
        '      "intent": "在项目删除后端链路中加入自动停止步骤。",',
        '      "assigneeRole": "executor",',
        '      "preferredBackend": "codex",',
        '      "scopeRead": ["packages/backend/src/repos"],',
        '      "scopeWrite": ["packages/backend/src/routes.ts"],',
        '      "acceptance": ["删除包含 active runs 的项目时不再返回 project has active runs。"],',
        '      "dependsOn": ["定位项目删除与 active runs 校验链路"]',
        '    }',
        '  ]',
        '}',
        '```',
      ...
      metadata: JSON.stringify({
        workflow_run_id: 'workflow-run-json-summary',
        workflow_step_id: 'workflow-step-json-summary',
        agent_run_id: 'agent-run-json-summary',
      }),
      created_at: now + 2_000,
    },
  ];
...
test('SessionShell keeps workflow chat layout compact by default', () => {
  ...
  assert.match(sessionOsCss, /\.deepsea-workflow-chat\s*\{[^}]*gap:\s*10px/s);
  assert.match(sessionOsCss, /\.deepsea-workflow-chat\s*\{[^}]*white-space:\s*normal/s);
  ...
  assert.match(sessionOsCss, /\.deepsea-message\.deepsea-message--workflow.is-merged-events \.deepsea-workflow-state-stream--events \.deepsea-workflow-state-step p\s*\{[^}]*display:\s*none/s);
  assert.match(sessionOsCss, /\.deepsea-message\.deepsea-message--workflow \.deepsea-workflow-state-step p\s*\{[^}]*-webkit-line-clamp:\s*2/s);
  ...
  assert.match(sessionOsCss, /\.deepsea-message\.deepsea-message--workflow:not\(\.is-merged-events\) \.deepsea-workflow-state-step:nth-child\(n \+ 3\)\s*\{[^}]*min-height:\s*0/s);
  assert.match(sessionOsCss, /\.deepsea-workflow-events__compact\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s+auto/s);
  assert.match(sessionOsCss, /\.deepsea-workflow-events__compact\s*\{[^}]*grid-template-areas:/s);
  assert.match(sessionOsCss, /\.deepsea-workflow-events__compact span\s*\{[^}]*-webkit-line-clamp:\s*2/s);
  assert.match(sessionOsCss, /\.deepsea-workflow-events__compact span\s*\{[^}]*white-space:\s*normal/s);
  assert.match(sessionOsCss, /\.deepsea-message\.deepsea-message--workflow \.deepsea-workflow-events\[data-compact="true"\]\s*\{[^}]*border-width:\s*1px 0 0/s);
  assert.match(sessionOsCss, /\.deepsea-message\.deepsea-message--workflow \.deepsea-workflow-events\[data-compact="true"\]\s*\{[^}]*background:\s*transparent/s);
...
test('SessionShell renders workflow controller and agent assignment table', () => {
  ...
  assert.match(html, /data-workflow-chat-message="true"/);
  assert.match(html, /data-workflow-assignment-summary="true"/);
  assert.doesNotMatch(html, /data-workflow-state-stream="true"/);
  assert.doesNotMatch(html, /deepsea-workflow-state-step/);
  assert.doesNotMatch(html, /data-workflow-flow-root="true"/);
...
test('SessionShell keeps workflow chat layout stacked like a chat message', () => {
  ...
  assert.match(sessionOsCss, /\.deepsea-workflow-state-stream__steps\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)/s);
  assert.match(sessionOsCss, /\.deepsea-workflow-state-stream--events \.deepsea-workflow-state-stream__steps\s*\{[^}]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/s);
  assert.match(sessionOsCss, /\.deepsea-workflow-state-step\s*\{[^}]*grid-template-columns:\s*18px\s+minmax\(0,\s*1fr\)/s);
  assert.match(sessionOsCss, /\.deepsea-workflow-state-step__icon\s*\{[^}]*display:\s*inline-flex/s);
  ...
  assert.match(sessionOsCss, /\.deepsea-message\.deepsea-message--workflow \.deepsea-workflow-state-step p\s*\{[^}]*display:\s*-webkit-box/s);
  assert.match(sessionOsCss, /\.deepsea-workflow-state-step p\s*\{[^}]*white-space:\s*normal/s);
