...
test('promote-to-workflow does not refresh replayed source task when workflow cannot start', async () => {
  const { room } = createCollaborationFixture('promote-active-no-mutation');
  const original = messageRepo.create({
    room_id: room.id,
    sender_type: 'user',
    sender_id: 'user',
  ...
  const oldDescription = '旧描述，没有产品经理方案背景。';
  const oldTask = taskRepo.create({
    room_id: room.id,
    project_id: room.project_id,
    title: '旧文件管理任务',
  ...
  const latestTask = taskRepo.get(oldTask.id);
  assert.equal(latestTask?.title, '旧文件管理任务');
  assert.equal(latestTask?.description, oldDescription);
});
