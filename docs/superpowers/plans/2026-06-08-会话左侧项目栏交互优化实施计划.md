...
- [x] **Step 3: 扩展 session PATCH schema**
...
- [x] **Step 6: 扩展 `api.updateSession` 入参类型**
...
- [x] **Step 4: 扩展 projectSwitcher 排序元数据**
...
- [x] **Step 6: 扩展 `SessionShell` 和 `SessionShellView` props**
...
- [x] **Step 1: 写 payload helper 失败测试**
...
- [x] **Step 4: 接入 mutation 状态和 Dialog**
...
执行完成后，将本计划中已完成步骤从 `- [x]` 改为 `- [x]`。不要勾选未执行命令。
...
- Task 1 已提交：`c785497 fix(session): 支持会话置顶更新`。
- Task 2 已提交并返修：`6fa003d`、`74af568`、`dbac621`、`b1287f7`、`411a17c`、`68c7b38`、`24af3cd`、`ba19d24`。
- Task 3 已提交并返修：`037973d`、`13faae9`。
- Task 4 已提交并返修：`0e40069`、`f585fd5`。
- 子任务复审结果：Task 2 质量复审 Approved；Task 3 规格和质量复审 Approved；Task 4 规格复审 Approved。
- 定向回归：`PATH=/Users/chendimao/.local/share/mise/installs/node/22.18.0/bin:$PATH node --import tsx --test packages/backend/src/session.routes.test.ts packages/frontend/src/lib/api.test.ts packages/frontend/src/pages/SessionWorkspacePage.test.tsx packages/frontend/src/session-ui/SessionShell.test.tsx packages/backend/src/session-workspace-view-model.test.ts`，结果 `109 pass / 0 fail`。
- 完整构建：`PATH=/Users/chendimao/.local/share/mise/installs/node/22.18.0/bin:$PATH npm run build`，结果通过；Vite 仅输出 chunk size warning。
- Diff 检查：`git diff --check`，结果通过，无输出。
- 浏览器 smoke：使用已运行的 `http://localhost:5173` 和后端 `7330` 打开真实会话页，验证 actions 默认隐藏、hover 显示、会话 pin hover 显示、当前会话浅灰高亮、时间列固定宽度。
- 截图：`output/playwright/session-sidebar-project-rail-final.png`。
