...
- [x] **Step 3: 扩展 page command helpers**
...
- [x] **Step 3: 扩展 SessionShell props**
...
- [x] **Step 4: 替换 project switcher 静态数据**
...
- [x] **Step 5: 替换 bottom status 静态数据**
...
- [x] **Step 7: 替换 plan/tools/diff fallback**
...
- [x] **Step 10: Compact discard button uses backend callback**
...
- [x] **Step 1: 修复 WebSocket 快速关闭 warning**
...
- [x] **Step 3: 运行 frontend targeted tests**
...
```bash
rtk git add \
  docs/superpowers/plans/2026-06-06-会话页面后端接入补齐实施计划.md \
...
```bash
rtk node --import tsx --test \
  packages/backend/src/session-types.test.ts \
  packages/backend/src/repos/session-contracts.test.ts \
  packages/backend/src/session-workspace-view-model.test.ts \
  packages/backend/src/repos/history-records.test.ts \
  packages/backend/src/session.routes.test.ts
...
```bash
rtk node --import tsx --test \
  packages/frontend/src/components/AppShell.test.tsx \
  packages/frontend/src/lib/ws.test.ts \
  packages/frontend/src/lib/sessionApi.test.ts \
  packages/frontend/src/lib/sessionTypes.test.ts \
  packages/frontend/src/session-ui/SessionShell.test.tsx \
  packages/frontend/src/session-ui/InspectorPanel.test.tsx \
  packages/frontend/src/pages/SessionWorkspacePage.test.tsx
```
...
- [x] **Step 5: Update final verification evidence**

### Final Verification Evidence

- Backend targeted tests: `rtk node --import tsx --test packages/backend/src/session-types.test.ts packages/backend/src/repos/session-contracts.test.ts packages/backend/src/session-workspace-view-model.test.ts packages/backend/src/repos/history-records.test.ts packages/backend/src/session.routes.test.ts`，结果：21/21 pass。
- Frontend targeted tests: `rtk node --import tsx --test packages/frontend/src/components/AppShell.test.tsx packages/frontend/src/lib/ws.test.ts packages/frontend/src/lib/sessionApi.test.ts packages/frontend/src/lib/sessionTypes.test.ts packages/frontend/src/session-ui/SessionShell.test.tsx packages/frontend/src/session-ui/InspectorPanel.test.tsx packages/frontend/src/pages/SessionWorkspacePage.test.tsx`，结果：22/22 pass；React Router server-render warning 为既有测试环境 warning。
- Build: `rtk npm run build`，结果：exit 0；后端 `tsc` 通过，前端 `vite build` 通过，仅 Vite chunk-size warning。
- Browser smoke: `http://localhost:5173/projects/b6p57AAqxbcl`，结果：fresh browser context 首屏 0 error，只有 2 条 React Router future flag warning；WebSocket immediate unsubscribe/reconnect warning 为 0。
- Browser functional flow: Contract scope/risks/acceptance 保存后 workspace payload 可读回；History 搜索请求成功；`/status` 返回 200；`/context` 返回 context manifest `SWEoof4o9_R8gjhE` 且 sourceCount=4；`/compact` apply 生成 `LYh66V_tRYGYGADi` 且 status=`applied`；`/compact` discard 生成 `m1Z5MsNzQcVqv8Ld` 且 status=`discarded`；Retry 创建 run `nf7AUiksXzcpa9SG`，Stop 返回 status=`cancelled`；普通消息 `浏览器验证消息 browser-1780685727302 请简短回复 OK` 创建 run `4OEEhTGaDfhje6mf` 且 status=`completed`；`/new` 从 `TBWTVjZNidu2-PXu` 切到 `Zt3UmxRPdu7mcURe`；History resume 到 `Zt3UmxRPdu7mcURe`；History fork 到 `6TWUP13D3iDOF_b5`。
- Final browser send/complete check: 通过真实页面输入框发送 `/new` 后进入 session `xDikK1Q_oyx_jZEw`，再通过输入框发送 `浏览器最终验证任务 1780687181691：请只回复 OK，不要修改文件。`；后端 run `7ctQn5yja0lxHegn` status=`completed`，stdout=`OK`，prompt 命中本次消息；页面可见该消息；控制台 0 error、0 WebSocket warning，仅 2 条 React Router future flag warning。
