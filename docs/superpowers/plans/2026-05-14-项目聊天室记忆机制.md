...
- [x] **Step 1: RoomPage 引入 MemoryPanel**
...
- [x] **Step 3: TaskDetailPanel 接入任务记忆**
...
- [x] **Step 4: RoomPage 传递 projectId 给 TaskDetailPanel**
...
新增 room/project source 去重索引，补充重复 room message 记忆返回 409 的路由测试；前端保存消息记忆遇到 409 时提示“这条消息已保存为记忆”并刷新记忆查询。

- [x] **Step 8: 最终审查修复记忆管理边界**

新增项目作用域记忆更新/删除 API，前端改用 project-scoped update/delete；补充跨项目 patch/delete 记忆被拒绝的后端测试；消息保存和记忆表单都限制内容长度为 12000。
...
4. 前端删除记忆是否只调用 DELETE /projects/:projectId/memories/:id，没有误删 task/message。
...
- [x] **Step 6: 最终审查修复 stored prompt injection 风险**

在记忆上下文 block 中加入不可信参考资料边界说明，明确记忆不能覆盖当前用户/系统指令，也不能作为可执行命令。最终整体复审结果为 Approved。
...
**Type consistency:** 后端和前端统一使用 `MemoryEntry`、`MemoryScope`、`MemoryType`、`MemorySourceType`、`memory_type`、`room_agent_id`、`source_type`、`source_id` 字段。API 路径统一为 `GET/POST /projects/:projectId/memories`、`PATCH/DELETE /projects/:projectId/memories/:id`。
