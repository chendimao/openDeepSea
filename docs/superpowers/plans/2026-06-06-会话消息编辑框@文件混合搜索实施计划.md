...
- [x] **Step 5: 创建 SessionFileComposer**
...
- [x] **Step 2: 写 dispatch metadata 失败测试**
...
- [x] **Step 5: 扩展 dispatchSessionUserMessage**
...
- [x] **Step 2: 写 runtime imagePaths 失败测试**
...
- [x] **Step 4: 实现 session-file-reference-context helper**
...
- [x] **Step 5: 接入 dispatch prompt 与 imagePaths**
...
- [x] **Step 6: 更新 dispatch prompt 测试**
...
- [x] `@` 混合搜索在 Session 消息编辑框可用。
- [x] 搜索结果按 `Source` / `Library` 分组，组标题不可被键盘选中。
- [x] chip 选择后发送 WebSocket payload 包含 `workspaceFileRefs` / `libraryFileRefs`。
- [x] slash command 不携带 refs。
- [x] 后端 message metadata 写入 `workspace_file_refs` / `library_file_refs`。
- [x] workspace 文本和 `agent_document` 按预算注入 prompt。
- [x] 上传文本文件默认只注入 metadata。
- [x] Source 图片路径传递到 `runSessionAgent.imagePaths`。
- [x] `npm run build` 通过。
